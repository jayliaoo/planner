...
pub(crate) async fn add(
    State(repository): State<Arc<SprintRepository>>,
    Json(payload): Json<AddSprintParam>,
) -> Json<MyResult<i32>> {
...
pub(crate) async fn get_all(
    State(repository): State<Arc<SprintRepository>>,
) -> Json<MyResult<Vec<Sprint>>> {
    let sprints = repository.get_list(&SprintListParam { ids: None }).unwrap();
